...
void initialize()
{
    ...
    if(strstr(iniRecv->metadata, "wait\0") != NULL)
    exit(0);
    do
    {
        if (strstr(iniRecv->metadata, "question\0") != NULL)
        {
...
void main()
{
    ...
    {
        printf("\n");
        me.clientSFD = createClient("127.0.0.1", 8001);
        counter++;
    ...
    pthread_create(&me.recvThread, NULL, (void *)senderFunction, NULL);

    struct message *recvBuffer = malloc(size_of_message);
    while (1)
    {
        recv(me.clientSFD, recvBuffer, size_of_message, 0);
        //if(strstr(recvBuffer->metadata, "log\0") == NULL && strstr(me.name, "admin\0") != NULL)
        printf("[%s] %s\n", recvBuffer->sender, recvBuffer->text);
        // else
        // printf("%s", recvBuffer->text);
    }
}